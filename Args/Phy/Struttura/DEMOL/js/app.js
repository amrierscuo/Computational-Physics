...
/*
  ...
  Nel quadrante Q1 c'è il loop fisico Lorenz-96 RK4, generato dai dati clean.

  Questa v5 conserva v4 e aggiunge un layer scientifico: live RMSE, timeline sync, dashboard metriche, tour mode e leggibilità migliorata.
  Però contiene già la logica che servirà dopo:
...
const TOUR_STEPS = [
  ...
  'Q4: Clean Hovmöller-only quadrant: no diagnostic walls, just the 3D terrain.',
];
...
async function init() {
  ...
  // v10 cleanup: remove the freestanding diagnostics signboard from the Hovmöller quadrant.
  ...
  // v10 cleanup: keep the 3D Hovmöller surface itself, but remove the extra label signboards around it.
...
function buildDiagnosticsWall(station) {
  ...
  addTextPlane(group, 'λ1 reference 1.68 · CNN next 1.81 · tendency ≈1.51', new THREE.Vector3(0, 9.7, -0.36), 7.6, 0.85, { color: '#d9ffc9', bg: 'rgba(0,0,0,0)' });
...
function addCentralInstructionLabels() {
  ...
  addTextPlane(root, 'Loop unico 15s: in futuro sincronizzerà GIF/grafici e modelli ML', new THREE.Vector3(0, 3.0, -9), 13.0, 0.9, { color: '#aeeeff', font: 30, bg: 'rgba(0,0,0,.16)' });
}
...
// -----------------------------------------------------------------------------
// V2 scientific additions: Lorenz-63 hero, Hovmöller 3D, and 2D display panels
// -----------------------------------------------------------------------------
...
class Hovmoller3DSurface {
  ...
  makeObjects() {
    ...
    addTextPlane(
      this.group,
      'L96 Hovmöller 3D terrain\nlocation × time × state amplitude',
...
class SyncedRolloutDashboardPanel {
  ...
  drawHovmoller(ctx, x, y, w, h, revealIdx) {
    this.drawPanelFrame(ctx, x, y, w, h, 'Progressive Hovmöller reveal');
...
class ModelMetricPanel {
  ...
  update(phase, elapsed) {
    ...
    ctx.fillText(`15s final RMSE ${fmtMetric(finalRmse)} · horizon ${fmtMetric(horizon)} t.u. · λ₁ ${fmtMetric(lambda)}`, 70, 332);
...
class DetailedDiagnosticsWall {
  ...
  draw() {
    ...
    this.drawBarGroup(ctx, 585, 230, 430, 260, 'Lyapunov λ₁', 'reference physical = 1.68', models, k => MODEL_META.lambda1?.[k], 2.0);
...
class V17DynamicLeaderboardWall {
  ...
  draw(phase, elapsed) {
    ...
    ctx.font = '800 34px Inter, Segoe UI, Arial';
    ctx.fillText('ranking updates by live RMSE · fixed λ₁/final metrics', 90, 162);
    ...
    ctx.font = '700 24px Inter, Segoe UI, Arial';
    ctx.fillText('Ranking updates by live RMSE; horizon, λ₁ and final RMSE stay fixed. Main showroom stays clean.', 90, 970);
...
class V11ProjectDChecklistWall {
  ...
  draw() {
    ...
    const items = [
      ...
      ['Lyapunov comparison', 'λ1 summary retained in diagnostics wall'],
    ...
    let y = 200;
    items.forEach((it, i) => {
      ctx.fillStyle = '#4dffb1'; ctx.font = '900 34px Inter, Segoe UI, Arial'; ctx.fillText('✓', 78, y + 10);
...
class V11ScientificLeaderboardWall {
  ...
  draw() {
    ...
    const cards = [
      ...
      ['Closest to physical λ₁ = ' + fmtMetric(physicalLambda), `${bestLambda.short} · |Δλ₁| = ${fmtMetric(bestLambda.lambdaErr)}`, bestLambda.color, 'Best chaos-metric agreement among the learned models.'],
    ...
    const headers = [
      ['Model', 90], ['Horizon (UM)', 390], ['λ₁', 620], ['|Δλ₁| vs phys', 760], ['Final RMSE', 970]
    ];
...
function onKeyDown(e) {
  ...
  if (e.code === 'BracketLeft') { state.speed = Math.max(0.1, state.speed - 0.1); showSyncToast(`Speed ${state.speed.toFixed(1)}×`); }
  if (e.code === 'BracketRight') { state.speed = Math.min(3.0, state.speed + 0.1); showSyncToast(`Speed ${state.speed.toFixed(1)}×`); }
}
...
function updateHud(elapsed, phase) {
  ...
  speedText.textContent = `${state.paused ? 'paused · ' : ''}speed ${state.speed.toFixed(1)}× · fly ${state.flyMode ? 'ON' : 'OFF'}${state.flyMode ? ` · alt ${controls.getObject().position.y.toFixed(1)}` : ''}`;
  ...
  if (dPhysical < 26) {
    note.innerHTML = '<b>Physical RK4:</b> verità di riferimento. Q2/Q3 usano rollout reali esportati dai checkpoint 100k.';
  } else if (pos.x > 18 && pos.z > 18) {
    note.innerHTML = '<b>Q4 pulito:</b> Hovmöller 3D terrain only, senza diagnostics wall o cartelli verdi.';
  } else if (pos.x > 18 && pos.z < -18) {
