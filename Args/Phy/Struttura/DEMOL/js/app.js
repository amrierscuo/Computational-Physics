...
function onMobileLookStart(e) {
  ...
  // actual look pad and empty canvas area to control yaw/pitch.
  const targetIsLookPad = mobileLookPad && e.target === mobileLookPad;
  const targetIsCanvas = e.target === canvas;
  if (mobileControls && mobileControls.contains(e.target) && !targetIsLookPad) return;
  if (!targetIsLookPad && !targetIsCanvas && e.target !== (mobileLookPad || canvas)) return;
  ...
  (targetIsLookPad ? mobileLookPad : canvas).setPointerCapture?.(e.pointerId);
}
