"""
Train a periodic 1-D CNN emulator on the Lorenz-96 small dataset.
...
Outputs:
    ...
    figures/cnn_l96_next_small_loss.png
    reports/cnn_l96_next_small_metrics.txt

Purpose:
    Compare a spatially-aware periodic CNN against the flat MLP baseline
    on Lorenz-96.
"""
...
from src.ml_models import PeriodicCNN1DEmulator, count_parameters


def load_dataset(path):
    """
    Load next-state supervised pairs from the L96 small dataset.
    """
    data = np.load(path)
...
def main():
    """
    Train the periodic CNN and save model, loss figure, and metrics.
    """
    data_path = Path("data/l96_small.npz")
    model_path = Path("models/cnn_l96_next_small.pt")
    figure_path = Path("figures/cnn_l96_next_small_loss.png")
    metrics_path = Path("reports/cnn_l96_next_small_metrics.txt")
    ...
    with open(metrics_path, "w") as f:
        f.write("Periodic CNN L96 next-state small dataset\n")
        f.write("=========================================\n")
